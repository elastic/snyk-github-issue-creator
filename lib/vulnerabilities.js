...
module.exports = async () => {
  ...
  const spinnerText = 'Loading Snyk issues'
  let totalIssues = 0
  ...
  for (const snykProject of conf.snykProjects) {
    ...
    for (const issue of issues) {
      ...
      spinner.text = `${spinnerText} (${processedIssues}/${++totalIssues})`
      queue.add(populateIssueDependencyGraph.bind(null, issue, page))
  ...
  async function populateIssueDependencyGraph (issue, page) {
