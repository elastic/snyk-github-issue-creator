...
--sortByPackageName, --no-sortByPackageName
                 If specified, Snyk vulnerabilities will be sorted by package
                 name when listing all vulnerabilities in batch mode. Default:
                 Sort by severity.
...
exports.init = async (args) => {
  ...
  Object.assign(
    ...
    await prompt([
      ...
      {
        type: 'confirm',
        name: 'sortByPackageName',
        message: 'Sort vulnerabilites by package name instead of severity',
        skip: shouldSkip('sortByPackageName'),
        initial: getConfig('sortByPackageName')
      },
      {
        type: 'confirm',
        name: 'autoGenerate',
